...
const ProfessionalInvoice = ({ invoiceData, invoiceId, isOffline = false }) => {
  ...
  const data = invoiceData?.data || invoiceData || {
    ...
    notes: "Thank you for choosing GreenGen Energy Solutions for your renewable energy needs."
  };

  const handlePrint = () => {
    // Simple print approach - open a new window with just the invoice content
                  ...
                  <tr>
                    <td>
                      ...
                      ${(item.zona_clim || item.climate_zone) ? 
                        `<br><small style="color: #2563eb;">Zone: ${item.zona_clim || item.climate_zone}</small>` : ''}
                    </td>
                    <td>
                      <strong>${item.descrizione_titolo || item.description || ''}</strong>
                      ${item.descrizione ? `<br><small>${item.descrizione}</small>` : ''}
                    </td>
            ...
            <div style="display: flex; justify-content: flex-end; margin: 30px 0;">
              <div style="width: 300px; border: 2px solid #ccc; border-radius: 8px; overflow: hidden;">
                ...
                <div style="display: flex; justify-content: space-between; padding: 10px; background: #f9fafb; border-bottom: 1px solid #ccc;">
                  ...
                  <span><strong>€${(data.tax_amount || 0).toFixed(2)}</strong></span>
                </div>
                <div style="display: flex; justify-content: space-between; padding: 15px; background: #086d32; color: white;">
                  ...
                  <span style="font-size: 18px; font-weight: bold;">€${(data.total_amount || 0).toFixed(2)}</span>
                </div>
              </div>
            </div>
    ...
    <div className="max-w-4xl mx-auto bg-white shadow-2xl overflow-hidden">
      ...
      >
        ...
        <div className="print-header bg-[#086d32] text-white p-6 h-24 flex items-center justify-between">
          ...
          <div className="rounded-lg flex items-center justify-center">
            <div className="bg-opacity-20 rounded flex items-center justify-center text-xs">
              <Image src="/images/logo.png" alt="Company Logo" width={150} height={120} />
            </div>
          </div>
        </div>
        ...
        <div className="print-main-content p-8">
          ...
          {/* Products Table with separate Product column */}
                ...
                {data.products?.map((item, index) => (
                  <tr key={`final-invoice-item-${item.product_id || index}`} className="border-b-2 border-gray-200 hover:bg-gray-50">
                    ...
                    <td className="py-3 px-3 border-r border-gray-200">
                      <div className="font-medium text-gray-900">{item.tipo || item.name || ''}</div>
                      {(item.zona_clim || item.climate_zone) && (
                        <div className="text-xs text-blue-600 mt-1 font-medium">Zone: {item.zona_clim || item.climate_zone}</div>
                      )}
                      {item.immagine && (
                        <div className="text-xs text-gray-500 mt-1">Image: {item.immagine}</div>
                      )}
                    </td>
                    ...
                    <td className="py-3 px-3 border-r border-gray-200">
                      {(item.descrizione_titolo || item.description) && (
                        <div className="font-medium text-gray-900">{item.descrizione_titolo || item.description}</div>
                      )}
                      {item.descrizione && (
                        <div className="text-sm text-gray-600 mt-1">{item.descrizione}</div>
                      )}
                    </td>
                    
                    {/* Quantity Column */}
                    <td className="py-3 px-3 text-center text-gray-900 border-r border-gray-200 font-medium">
                      {item.quantity}
                    </td>
                    
                    {/* Unit Price Column */}
                    <td className="py-3 px-3 text-right text-gray-900 border-r border-gray-200">
              ...
              <div className="border-2 border-gray-300 rounded-lg overflow-hidden">
                <div className="space-y-0">
                  <div className="flex justify-between py-3 px-4 border-b border-gray-200 bg-gray-50">
                    ...
                    <span className="text-gray-900 font-semibold">€{(data.subtotal || data.total || 0).toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between py-3 px-4 border-b border-gray-200 bg-gray-50">
                    <span className="text-gray-700 font-medium">Tax ({((data.tax_rate || 0) * 100).toFixed(1)}%):</span>
                    <span className="text-gray-900 font-semibold">€{(data.tax_amount || 0).toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between py-4 px-4 bg-green-800 text-white">
                    <span className="text-lg font-bold">Total Amount:</span>
                    <span className="text-lg font-bold">€{(data.total_amount || data.total || 0).toFixed(2)}</span>
                  </div>
                </div>
              </div>
            </div>
