          ...
          <>
            ...
            <div className="block lg:hidden space-y-3">
              {invoiceData.products.map((item, index) => (
                <div
                  key={`preview-mobile-item-${item.product_id || index}`}
                ...
                >
                  <div className="font-medium text-gray-900 dark:text-white text-sm mb-2">
                    ...
                    {(item.climate_zone || item.zona_clim) && (
                      <div className="text-xs text-blue-600 dark:text-blue-400 font-normal mt-1">
                        Zone: {item.climate_zone || item.zona_clim}
                      </div>
                    )}
                  </div>
              ...
              <table className="w-full border-collapse min-w-[480px]">
                <thead>
                  <tr className="border-b-2 border-gray-200 dark:border-gray-700">
                    <th className="text-left py-2 text-xs lg:text-sm font-medium text-gray-700 dark:text-gray-300">
                      Item
                    </th>
                ...
                <tbody>
                  {invoiceData.products.map((item, index) => (
                    <tr
                      key={`preview-item-${item.product_id || index}`}
          ...
          <div className="w-64 space-y-2">
            <div className="flex justify-between">
              ...
              <span className="text-gray-900 dark:text-white">
                €{(invoiceData?.subtotal || invoiceData?.total || 0).toFixed(2)}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600 dark:text-gray-400">
                Tax (
                {(
                  (invoiceData?.tax_rate || invoiceData?.taxRate || 0.085) * 100
                ).toFixed(1)}
                %):
              </span>
              <span className="text-gray-900 dark:text-white">
            ...
            <div className="flex justify-between font-bold text-lg border-t border-gray-200 dark:border-gray-700 pt-2">
              ...
              <span className="text-gray-900 dark:text-white">
                €
                {(invoiceData?.total_amount || invoiceData?.total || 0).toFixed(
                  2
                )}
              </span>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                handleInputChange("City", e.target.value, "building_site")
              }
              className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-white"
              placeholder="City"
              disabled={isOffline}
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                handleInputChange("Postal", e.target.value, "building_site")
              }
              className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-white"
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                handleInputChange("Country", e.target.value, "building_site")
              }
              className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-white"
              placeholder="Country"
              disabled={isOffline}
